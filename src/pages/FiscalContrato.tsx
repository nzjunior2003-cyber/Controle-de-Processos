...
import { 
  ...
  ShieldAlert, AlertCircle, FileText, ShieldCheck, X, Upload, ChevronDown, ChevronUp, FileCheck
} from 'lucide-react';
...
export default function FiscalContrato() {
  ...
  const [execucoes, setExecucoes] = useState<any[]>([
    { id: 1, contratoId: '1', tipo: 'NF/Fatura', nf: '001234', data: '2025-07-15', valor: 50000, quantidade: 1, observacao: 'Faturamento ref ao mês de Junho/2025', arquivoLink: null },
    { id: 2, contratoId: '1', tipo: 'NF/Fatura', nf: '001289', data: '2025-08-16', valor: 45000, quantidade: 1, observacao: 'Faturamento ref ao mês de Julho/2025', arquivoLink: null },
  ]);
  const [novaNota, setNovaNota] = useState<{ tipoDeducao: 'valor' | 'quantidade', tipo: string, nf: string, data: string, valor: string, quantidade: string, observacao: string, arquivo: File | null }>({ tipoDeducao: 'valor', tipo: 'NF/Fatura', nf: '', data: '', valor: '', quantidade: '1', observacao: '', arquivo: null });
  const [isUploading, setIsUploading] = useState(false);
// Removed abaAtiva and abaModal state. Keep only geral view and execucao modal view.
  ...
  const handleAddNota = async () => {
    if (!novaNota.nf || !novaNota.data) return;
    if (novaNota.tipoDeducao === 'valor' && !novaNota.valor) return;
    if (novaNota.tipoDeducao === 'quantidade' && !novaNota.quantidade) return;
    ...
    try {
      ...
      if (token && novaNota.arquivo) {
        ...
        const metaRes = await fetch('https://www.googleapis.com/drive/v3/files', {
          ...
          body: JSON.stringify({
            name: `NF_${novaNota.nf}.pdf`,
            mimeType: novaNota.arquivo.type || 'application/pdf'
          })
        });
        
        if (!metaRes.ok) throw new Error('Falha ao criar arquivo no Drive');
        ...
        const uploadRes = await fetch(`https://www.googleapis.com/upload/drive/v3/files/${fileId}?uploadType=media`, {
          ...
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': novaNota.arquivo.type || 'application/pdf'
          },
          body: novaNota.arquivo
        });
        
        if (!uploadRes.ok) throw new Error('Falha no upload do arquivo');
        arquivoLink = `https://drive.google.com/file/d/${fileId}/view`;
      }
      ...
      setExecucoes([...execucoes, {
        ...
        quantidade: Number(novaNota.quantidade) || 1,
        observacao: novaNota.observacao,
        arquivoLink
      }]);
      setNovaNota({ tipoDeducao: 'valor', tipo: 'NF/Fatura', nf: '', data: '', valor: '', quantidade: '1', observacao: '', arquivo: null });
    } catch (error) {
      console.error(error);
  ...
  const filtrados = contratosPermitidos.filter(c => 
  ...
  const isMasterOrFiscal = usuarioAtual?.perfil === 'master' || usuarioAtual?.perfil === 'fiscal';
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Bem-vindo, {usuarioAtual?.nome}. Abaixo estão os contratos sob sua responsabilidade.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
         <div className="bg-white p-5 rounded-lg border border-gray-200 shadow-sm border-l-4 border-l-emerald-500">
         ...
         <div className="bg-white p-5 rounded-lg border border-gray-200 shadow-sm border-l-4 border-l-amber-500">
           <div className="flex justify-between items-start">
              <div>
         ...
         <div className="bg-white p-5 rounded-lg border border-gray-200 shadow-sm border-l-4 border-l-red-600">
           <div className="flex justify-between items-start">
              <div>
                ...
                <div className="lg:col-span-1">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <select 
                        value={novaNota.tipoDeducao}
                        onChange={(e) => setNovaNota({...novaNota, tipoDeducao: e.target.value as 'valor' | 'quantidade'})}
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Identificação do Doc. (Nº NF / Recibo)</label>
                      <input 
                        type="text" 
                        value={novaNota.nf}
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Data do Faturamento / Recibo</label>
                      <input 
                        type="date" 
                        value={novaNota.data}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {novaNota.tipoDeducao === 'quantidade' && (
                        <div>
                          <label className="block text-sm font-medium text-gray-700">Qtd. Utilizada (Obrigatório)</label>
                          <input 
                            type="number" 
                            value={novaNota.quantidade}
                            onChange={(e) => setNovaNota({...novaNota, quantidade: e.target.value})}
                            className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm" 
                          />
                        </div>
                      )}
                      <div className={novaNota.tipoDeducao === 'valor' ? 'col-span-2' : ''}>
                        <label className="block text-sm font-medium text-gray-700">Valor (R$) {novaNota.tipoDeducao === 'valor' ? '' : '(Opcional)'}</label>
                        <input 
                          type="number" 
                    ...
                    <button 
                      ...
                      disabled={isUploading || !novaNota.nf || !novaNota.data || (novaNota.tipoDeducao === 'valor' ? !novaNota.valor : !novaNota.quantidade)}
                            ...
                            <div className="flex justify-between items-start mb-2">
                              <div>
                                <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-800 border border-gray-200">
                                  {exec.tipo}: {exec.nf} (Qtd: {exec.quantidade})
                                </span>
